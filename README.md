...
**Recommended production posture.** Use `--output-mode file` to write the SQL out, review it, and apply it deliberately; reserve direct deploy (`--yes`) for local/dev iteration. Deploy through a **least-privilege database role** — one that can `CREATE FUNCTION` in the target schema but cannot `DROP`/`TRUNCATE` your tables or touch unrelated objects — so a destructive body cannot do damage even if it slips through review. (A smoked read runs inside the rolled-back deploy transaction, so a destructive body would not be exercised at deploy time; it would run on the first real call in production.)
