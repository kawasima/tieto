...
public final class OwnershipMarker {
    ...
    /** Whether a {@code pg_proc} comment marks the function as tieto-generated (by any repository). */
    public static boolean isManaged(String comment) {
        return comment != null && comment.startsWith(PREFIX);
    }

    /**
     * Whether the comment marks the function as generated by <em>this</em> repository. Prune must
     * check the {@code repository=} field, not just the {@code tieto:generated} prefix: a sibling
     * repository with an overlapping snake prefix (e.g. {@code Order} sweeping in {@code OrderItem}'s
     * {@code order_item_*}) carries its own marker, and dropping it would delete another
     * repository's live functions.
     */
    public static boolean isManagedBy(String comment, String repositorySimpleName) {
        return isManaged(comment) && repositorySimpleName.equals(repositoryOf(comment));
    }

    /** The {@code repository=} value recorded in the marker, or null if absent/unmarked. */
    static String repositoryOf(String comment) {
        if (comment == null) {
            return null;
        }
        for (String token : comment.split("\\s+")) {
            if (token.startsWith("repository=")) {
                return token.substring("repository=".length());
            }
        }
        return null;
    }
